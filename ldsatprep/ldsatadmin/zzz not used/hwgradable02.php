...
put_ptts_header("Gradable Homework for class ".$class_id, $strAbsPath, "admin", "index");
$order = $_REQUEST['order'];
$sort = $_REQUEST['sort'];
$move_id = $_REQUEST['move_id'];
$tablename = "TP_HW_Gradable";
if ($sort == "")
    $sort = "due_date";
if($order==""){
    $order="ASC";
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
      <td align="right" style="padding-bottom:5px"><button onclick="javascript:popup('gethwgradable.php?class_id=<?=$class_id?>&popup=1','Details','600','600')" style="cursor:pointer">Add Gradable Homework</button></td>
  </tr>
  <tr height="40">
    <td class="td_header">Gradable Homework for class <?=$class_id?></td>
  </tr>
  <tr>
  ...
  <tr style="background: #eee; height: 35px">
      ...
      <td nowrap><?php  echo '<a class=a_grey href="?class_id='.$class_id."&sort=due_date&".($order == "ASC" ? "&order=DESC" : "order=ASC").'" style="text-decoration:none"><b>Due date</b>'.($sort == 'due_date' ? ' <img border=0 src="'.$strAbsImgPath.'/order_'.($order == "ASC" ? "down" : "up").'.png">' : '').'</a>';?></td>
       <td nowrap><?php  echo '<a class=a_grey href="?class_id='.$class_id."&sort=name_report&".($order == "ASC" ? "&order=DESC" : "order=ASC").'" style="text-decoration:none"><b>Test</b>'.($sort == 'name_report' ? ' <img border=0 src="'.$strAbsImgPath.'/order_'.($order == "ASC" ? "down" : "up").'.png">' : '').'</a>';?></td>
       <td nowrap><?php  echo '<a class=a_grey href="?class_id='.$class_id."&sort=section&".($order == "ASC" ? "&order=DESC" : "order=ASC").'" style="text-decoration:none"><b>Section</b>'.($sort == 'section' ? ' <img border=0 src="'.$strAbsImgPath.'/order_'.($order == "ASC" ? "down" : "up").'.png">' : '').'</a>';?></td>
      <td class="text_grey"><b>Action</b></td>
  </tr>

<? 

$QStr = "select a.*, b.name_report from $tablename a LEFT JOIN TP_SAT_Tests b ON a.test=b.id  WHERE a.class_id='".$class_id."' order by ".($sort!="id" ? "trim($sort)" : "$sort")." $order,test, section";
$RS = runquery($QStr);
while($row = mysql_fetch_array($RS)){
 echo '<tr>
         ...
         <td nowrap  align=center>
             <a onclick="javascript:popup(\'gethwgradable.php?id='.$row[id].'&popup=1\',\'Details\',\'600\',\'600\')"><img SRC="'.$strAbsImgPath.'edit_pencil.gif" ALT="edit" border="0"></a>&nbsp;
