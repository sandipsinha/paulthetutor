...
<table width="100%" border="0" cellpadding="7" cellspacing="0" style="border:solid 1px #999">
 ...
 <?
 $SSQstr = "SELECT *  FROM TP_SAT_Scores WHERE student_id='".$sat_id."' AND test_id='".$test_id."' LIMIT 1";
 
 $res_score = runquery("SELECT *  FROM TP_SAT_Scores WHERE student_id='".$sat_id."' AND test_id='".$test_id."' LIMIT 1");
 ...
 ?>
  ...
  <tr>
    ...
    <td class="news_header"><?=$row_score['math_reported'];?></td>
  </tr>
  <tr>
    <td><div align="right"  class="news_header">Reading: </div></td>
    <td class="news_header"><?=$row_score['reading_reported'];?></td>
  </tr>
  <tr>
    <td><div align="right" class="news_header">Writing: </div></td>
    <td class="news_header"><?=$row_score['writing_reported'];?></td>
  </tr>
  <tr>
    <td><div align="right"  class="news_header">Essay: </div></td>
    <td class="news_header"><?=$row_score['essay_score'];?></td>
  </tr>
</table></td>
</tr>
<? 
...
<?
    $res = runquery("select * from $strTableName WHERE section='$secnum' AND test='$test_id' AND sid='$sat_id' ORDER BY problem ASC");
