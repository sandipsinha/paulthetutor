...
//assign 'where' if given variables exist
if(!IsEmpty($x_conf_class)) {
  $where = $where .  " or class_id='$x_conf_class'";

}

if(!IsEmpty($student_id)) {
  $where = $where . " or student_id='$student_id'";
}

// if there is hw assigned for a student, or a class that the student is in, show it
$HWSQStr = "select count(*) from TP_HW_Summary $where order by due_date desc";
...
?>
  ...
  <? 
  //$HWSQStr = "select * from TP_HW_Summary WHERE class_id='$x_conf_class' or student_id = $student_id order by due_date desc";
  $HWSQStr = "select * from TP_HW_Summary $where order by due_date desc";
