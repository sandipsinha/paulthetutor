...
<!--Added the form tag  Sandip on 10/21/14-->
<form>
<!--Updated the field to give it a id Sandip on 10/21/14-->
...
$students_name = get_family_snames($row['id']);
 ...
 echo '<tr id="family-table-row-'.$row['id'].'" class="familylist">
 		...
 		<td valign=top>'
						."Email: ".$email . '<br>' 
						."Phone: ".$phone .'</td>';
		create_comment('PT_Comments_Family','family_id', $row['id']); //table, column name, row id
 	...
 	if ($folder != "tutors"){
		...
		<a target=_blank href="studentinfo.php?fid='.$row[id].'"><img SRC="../images/roster_users1.png" ALT="View Students" title="Students" border="0"></a>&nbsp;
 		<a href="javascript:void(\'\')" onClick="if (confirm(\'Are you sure you want to archive this family?\')) {document.getElementById(\'move_id\').value='.$row['id'].'; document.forms[0].submit();}"><img SRC="../images/del_x.gif" ALT="archive" title="archive" border="0"></a>&nbsp;
...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST['notes'] != "") {
		add_comment("PT_Comments_Family", 'family_id' ,$_POST['id'], $_POST['notes']); //table, column name,row_id, comment
}
