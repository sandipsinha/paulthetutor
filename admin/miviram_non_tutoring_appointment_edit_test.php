	...
	//--- end of above
				...
				if ($_REQUEST['day_of_week']!=$arFieldsVals['day_of_week']){
	 				...
	 				foreach ($arr_days as $k=>$v){
	 	 				    $_REQUEST['date'] = $v;
							if($_REQUEST['paid']=='Paid')
						  	sri_non_tut_session_add($v, $_REQUEST['start_time'],$_REQUEST['end_time'], $_REQUEST['tutor_id'],$_REQUEST['name'], $_REQUEST['email'], $_REQUEST['phone'], $_REQUEST['comments'], $_REQUEST['sched_id'],1, $_REQUEST['rate']);
							else
							sri_non_tut_session_add($v, $_REQUEST['start_time'],$_REQUEST['end_time'], $_REQUEST['tutor_id'],$_REQUEST['name'], $_REQUEST['email'], $_REQUEST['phone'], $_REQUEST['comments'], $_REQUEST['sched_id'],0, '');
	 				 }
	 	 ...
	 	 foreach ($arr_days as $k=>$v){
			...
			// die();
			 if($_REQUEST['paid']=='Paid')
			 	...
			 	sri_non_tut_session_add($v, $_REQUEST['start_time'],$_REQUEST['end_time'], $_REQUEST['tutor_id'],$_REQUEST['name'], $_REQUEST['email'], $_REQUEST['phone'], $_REQUEST['comments'], $_REQUEST['sched_id']);
