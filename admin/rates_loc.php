...
If(!(empty($move_id))){
	$MQStr = "select id, rate, pay, description, (select short_name from PT_Locations where PT_Locations.id = PT_Rates_Loc.location_id) as location from PT_Rates_Loc where id=$move_id and exists(select 'x' from $tablename where id=$move_id) ";
	$MRS = runquery($MQStr);
	if($MAR = mysql_fetch_assoc($MRS)){
	//Added by Sandip on 10/15/14*********************Begin*****************
		$tablename = "PT_Rates";
	    $DQStr = "delete from $tablename where id = $move_id";
		runquery($DQStr);
	//Added by Sandip on 10/15/14*********************End*****************	
...
//Added by Sandip on 10/15/14*********************Begin*****************	
$where = " where exists(select 'x' from $tablename where id=PT_Rates_Loc.id)";
//Added by Sandip on 10/15/14*********************End*****************
