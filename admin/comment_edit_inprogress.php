...
if ($folder == "tutors"){
	...
	$_REQUEST[commentor] = get_tutor_name($tutor_id);
}
...
if(isEmpty($_REQUEST['date'])) {
	...
	//insert	
	 elseif ($_REQUEST['action'] == "insert"){
		...
		$_REQUEST_I[commentor] = $tutname;
		
//	echo "tut name is $_REQUEST[commentor] Req and name $tutname<BR>";	
...
// send a mail notification about the comment
	...
	$msubject = "$tutname posted a note titled '$_REQUEST[title]' about $student_name";
	$mmessage = "$msubject
	
	$_REQUEST[comment]";
	$strHeader = "From: PTTS Notes<notes@paulthetutors.com>\r\ncc:paul@paulthetutors.com";
...
//IF THIS IS AN EDIT, AND NOT AN INSERT, PUT THE DATE FIELD HERE
	...
	$strNotUsed = "id,tutor_id, student_id,reply_to";
