...
if(isset($_REQUEST['action']) && $_REQUEST['action'] && isset($_REQUEST['strTableName'])){
	...
	if($_REQUEST2['end_date']=='')
	$_REQUEST2['end_date']=$_REQUEST2['start_date'];
	//--- end of above
		//echo "trace";
	//die();
	//update
	...
	//insert	
	 elseif ($_REQUEST['action'] == "insert"){
		...
		if(isset($repeatedevent))  // if it is the repeated event
		{	
	...
	// Use this version if it is a paid session non_tut_session_add($v, $_REQUEST['start_time'],$_REQUEST['end_time'], $_REQUEST['tutor_id'],$_REQUEST['name'], $_REQUEST['email'], $_REQUEST['phone'], $_REQUEST['comments'], $_REQUEST['sched_id'],1, $RATE);			 
		...
		else   // if it is one time event
		{						
			if($_REQUEST['paid']=='Paid')
					 non_tut_session_add($_REQUEST['start_date'], $_REQUEST['start_time'],$_REQUEST['end_time'], $_REQUEST['tutor_id'],$_REQUEST['name'], $_REQUEST['email'], $_REQUEST['phone'], $_REQUEST['comments'], '0',1, $_REQUEST['rate']);			 				 
				 else
					non_tut_session_add($_REQUEST['start_date'], $_REQUEST['start_time'],$_REQUEST['end_time'], $_REQUEST['tutor_id'],$_REQUEST['name'], $_REQUEST['email'], $_REQUEST['phone'], $_REQUEST['comments'], '0');
...
putSelectInputOnChange('Paid Work<font color="#FF0000">*</font>&nbsp;&nbsp;&nbsp;', 'paid', $arr_paid, ($result_array['paid']==1?"Paid":"Unpaid"), '', '','',' onchange="showrate();"');
